...
```
同一个 prompt →
  Codex.ai Deep Research
  Gemini Deep Research
  ChatGPT Deep Research  ← 可能先问澄清问题，答完后把 Q&A 追加到另外两路的 prompt
```
结果存：*(internal reference removed)*（chatgpt / Codex-ai / gemini）
...
| Codex / Gemini Deep Research | Plan-dependent，同上 |
| GPT-5.2 Pro | 仅用于 Step 3 审阅，不用于普通对话 |

**三个视角的必要性：** Codex / Gemini / GPT 各家族有不同的训练偏差。分歧处往往是最有价值的信号。
...
- **Codex.ai** → `refs/Codex-ai-browser-automation.md`（2026-03-10 实测验证 ✅）
...
| 发送 | 点击发送按钮 | 输入框右侧蓝色箭头 |
| 确认计划 | 点击「开始研究」 | ← Gemini 独有！ChatGPT/Codex 无此步骤 |
...
- **✅ Codex.ai 报告**：Artifact 面板原生 "Download as Markdown" 按钮（blob URL，同源 DOM）
  - 点击 Copy options → Download as Markdown → 自动下载 .md 文件
  - 比 ChatGPT 简单得多——无需 API 提取（详见 `refs/Codex-ai-browser-automation.md`）
- **✅ Gemini 报告**：导出到 Google Docs → 文件 → 下载 → Markdown (.md)
  - 两跳路径（Gemini → Google Docs → 本地），比 ChatGPT/Codex 多一步
...
**场景**：需要咨询无法访问本地文件的云端模型（如 GPT Pro、Codex Pro 等）。
...
- `2026-03-05-mcp-security-Codex-pro-consult.md`
